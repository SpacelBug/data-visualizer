...
const xScale = computed(() => {
  if (props.params.xScaleTypes === 'linear' && svg.value) {
    logger.log('INFO', `init xScale (${props.params.xKey}| Dots chart)`)
...
const yScale = computed(() => {
  if (props.params.yScaleTypes === 'linear' && svg.value) {
    if (props.params.yKeys) {
      logger.log('INFO', 'init yScale (Dots chart)')
  ...
  if (props.data && yScale.value && xScale.value) {
    ...
    for (const key of props.params.yKeys) {
      d3.select(svg.value)
        ...
        .selectAll('circle')
        .data(props.data.map((d) => d[key]).filter((v) => v != null))
        .join('circle')
        .attr('cx', (d, i) =>
          props.params.xKey ? xScale.value(d[props.params.xKey]) : xScale.value(i),
        )
    ...
    logger.log('INFO', 'Dots chart plotted')
  }
}
</script>
